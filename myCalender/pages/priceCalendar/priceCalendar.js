...
Page({
  ...
  getClier() {
    wx.request({
      url: 'https://www.samewarm.com/Sandy/v2.0/userroom/halfDaysStatus?roomId=24',
      success: res => {
        let objsto = []
        let _halfMonth = res.data.Value;
        _halfMonth.sort(function(a, b) { //按月份儿排序
  ...
  chooseDate(e) {
    ...
    let RoomState_click = e.currentTarget.dataset.roomstate
    let date_click = e.currentTarget.dataset.date //获取点击的日期Date
    console.log(number_click, RoomState_click, date_click, this.data.nowDate)
    // 如果是空格或者以前的日期就直接返回
    if (RoomState_click === "out" || number_click === '' || date_click < this.data.nowDate) {
      console.log("不能选")
      return;
    }
  ...
  chooseHours(e) {
    ...
    let timestatus_click = e.currentTarget.dataset.timestatus
    console.log(date_click, hourtime_click, timestatus_click)
    if (timestatus_click === false || date_click < this.data.nowDateHour) {
      console.log("不能选")
      return;
    }
